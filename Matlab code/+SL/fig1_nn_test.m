...
gtDir = 'D:\NN workspace\data\custom cropped 320x320\ground truth 4pts';
if ~exist(gtDir, 'dir')
    warning('The path in NN workspace is not found');
    return
end

gtSearch = MBrowse.Dir2Table(fullfile(gtDir, '* ground truth.mat'));
gtPaths = fullfile(gtDir, gtSearch.name);
...
% Report numbers
fileID = fopen(fullfile(figDir, 'nn sample size.txt'), 'w');

fprintf(fileID, 'Classification: %i frames for training, %i frames for testing\n', ...
    sum(cellfun(@numel, tbAll.image(indTrain))), ...
    sum(cellfun(@numel, tbAll.image(indTest))));

fprintf(fileID, 'Regression: %i frames for training, %i frames for testing\n', ...
...
% Plot
f = MPlot.Figure(8484); clf
h = heatmap({'Tongue In', 'Tongue Out'}, {'Tongue In', 'Tongue Out'}, 100 * confMat);
h.XLabel = 'Predicted Class';
h.YLabel = 'True Class';
h.Title  = 'Normalized Confusion Matrix (%)';
MPlot.Paperize(f, 'ColumnsWide', .5, 'AspectRatio', .75);
saveFigurePDF(f, fullfile(figDir, 'nn classification'));
...
% Report numbers
fileID = fopen(fullfile(figDir, 'nn reg perf.txt'), 'w');
fprintf(fileID, 'Error in angle %.2g±%.2g° mean±SD\n', mean(Adiff), std(Adiff));
fprintf(fileID, 'Error in length %.2g±%.2g mm mean±SD\n', mean(Ldiff), std(Ldiff));
fclose(fileID);
